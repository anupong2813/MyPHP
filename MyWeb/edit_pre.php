...
if ($errors) {
    $_SESSION['sticky'] = compact('productname','detail','price','img_url','color','format','released_date','spotify_url');
    die('<h3>พบข้อผิดพลาด</h3><ul><li>'.implode('</li><li>',$errors).'</li></ul><a href="edit.php?id='.$id.'">ย้อนกลับไปแก้ไข</a>');
}

$_SESSION['preview'] = compact('id', 'productname','detail','price','preview_img','source','keep_old_image','old_img','color','format','released_date','spotify_url');

// ฟังก์ชันสำหรับจัด Path ของรูปที่จะ Preview
...
<div class="container">
    ...
    <div class="preview-layout">
        <div class="preview-img">
            <?php if ($preview_img): ?>
                <img src="<?= e(getPreviewImg($preview_img)) ?>" alt="Preview">
            <?php else: ?>
